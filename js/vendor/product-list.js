$(document).on('click','.product-status',function(){
	if($(this).text() == 'Active'){
		$(this).removeClass('bg-success-new');
		$(this).addClass('bg-danger-new');
		$(this).text('Deactive');
	}
	else if($(this).text() == 'Deactive'){
		$(this).addClass('bg-success-new');
		$(this).removeClass('bg-danger-new');
		$(this).text('Active');
	}
});
...
$('#leftside-menu-container a[data-target="product-overview"]').addClass('active');